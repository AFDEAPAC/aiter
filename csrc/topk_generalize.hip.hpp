...
__global__ void phase_small_n_topk(const float* __restrict__ input, int pitch,
  ...
  uint32_t pivot;
  int eq_needed;
  block_select_lds(s_keys, len, k_out, s_hist, s_red, s_scan, s_mm, pivot, eq_needed, npasses, true);
...
#endif
#if ABLATE_DRAIN == 1
#define COOP_DRAIN_BODY(off)   for (int _j = lane; _j < bcnt; _j += WAVE_SIZE) row_base[(off) + _j] = 1ull
#elif ABLATE_DRAIN == 2
#define COOP_DRAIN_BODY(off) do { } while (0)
#else
#define COOP_DRAIN_BODY(off)   for (int _j = lane; _j < bcnt; _j += WAVE_SIZE) row_base[(off) + _j] = buf[_j]
#endif

#define COOP_DRAIN_WAVE()                                                          \
  do {                                                                             \
    unsigned _off = 0;                                                             \
    if (lane == 0) _off = COOP_RESERVE(row, bcnt);                                   \
  ...
  if (!RAGGED) {
    ...
    if (ncols > 0 && bid == G - 1 && bcnt >= 0) {
      ...
      if (t > 0) {
        if (act)
          buf[bcnt + __popcll(bt & lt)] =
              ((uint64_t)__float_as_uint(vv) << 32) | (uint32_t)(tail0 + (int)threadIdx.x);
        bcnt += t;
      }
    }
  }
...
#define ABLATE_PC 0
  ...
  (void)npasses;
#else
  block_select_lds(s_keys_ext, c, k_out, s_hist, s_red, s_scan, s_mm, pivot, eq_needed,
                   npasses, true, PC_FOLD != 0);
#endif
