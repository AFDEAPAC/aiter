...
template <bool RAGGED> struct RowExtents;
template <> struct RowExtents<false> {
  const int* ends;
...
template <> struct RowExtents<true> {
  const int* starts;
  const int* ends;
...
__device__ __forceinline__ int row_len_of(int row, int pitch, RowExtents<RAGGED> ext) {
  if constexpr (RAGGED) return ext.row_len(row, pitch);
  return pitch;
}

__device__ __forceinline__ int n4_cover(int len) { return (len + FP32_EPT - 1) / FP32_EPT; }

__device__ __forceinline__ int k_take_dev(int K, int len) {
  return len < K ? len : K;
}
...
struct TopkOut<false> {
  int* idx;
  __device__ __forceinline__ int* idx_row(int row, int K) const {
    return idx + (size_t)row * K;
  }
...
// candidate's slot.
template <bool WRITE_VALUES>
__device__ __forceinline__ void pad_topk_tail(int* __restrict__ out, float* __restrict__ out_val,
                                              int k_take, int K) {
  for (int i = k_take + threadIdx.x; i < K; i += blockDim.x) {
    out[i] = -1;
    if (WRITE_VALUES) out_val[i] = -__builtin_inff();
  }
}
...
__device__ __forceinline__ void emit_identity_row(int* __restrict__ out,
                                                  ...
                                                  const float* __restrict__ row, int row_start,
                                                  int len, int K) {
  for (int i = threadIdx.x; i < K; i += blockDim.x) {
    const bool live = (i < len);
    out[i] = live ? (row_start + i) : -1;
    if (WRITE_VALUES) out_val[i] = live ? row[i] : -__builtin_inff();
  }
}
...
__device__ __forceinline__ vfloat4 load_row_f4(const float* __restrict__ row, int i, int len) {
  ...
  if constexpr (!RAGGED) {
    ...
    if constexpr (NT) return __builtin_nontemporal_load(v4 + i);
    else return load_f4(v4 + i);
  } else {
...
__device__ __forceinline__ void block_gather_topk(int c, uint32_t pivot, int ngt, int eq_needed,
  ...
  for (int i0 = 0; i0 < c; i0 += blockDim.x) {
    ...
    if (gt) {
      ...
      if (p < (unsigned)ngt) {
        out[p] = idx_at(i);
        if (WRITE_VALUES) out_val[p] = sortable_to_fp32(k);
      }
    }
    if (eq) {
      ...
      if (p < (unsigned)eq_needed) {
        out[ngt + p] = idx_at(i);
        if (WRITE_VALUES) out_val[ngt + p] = sortable_to_fp32(k);
      }
    }
  }
}
...
__device__ __forceinline__ void hist_add_aggregated(uint32_t* __restrict__ s_hist, uint32_t bucket,
                                                    int rep, bool active, int rounds) {
  const int lane = threadIdx.x & (WAVE_SIZE - 1);
  ...
  for (int r = 0; r < rounds && todo != 0ull; r++) {
    ...
    if (lane == leader) atomicAdd(&s_hist[cand * HIST_REP + rep], (uint32_t)__popcll(grp));
    todo &= ~grp;
  }
  if (todo & (1ull << lane)) atomicAdd(&s_hist[bucket * HIST_REP + rep], 1u);
}
...
__device__ __forceinline__ void block_find_pivot_bucket_wave0(uint32_t* __restrict__ s_hist,
...
#pragma unroll
      for (int r = 0; r < HIST_REP; r++) {
        ...
        if constexpr (CLEAR) s_hist[b * HIST_REP + r] = 0u;
      }
      v[j] = s;
      tot += s;
    }
...
template <bool CLEAR = false>
__device__ __forceinline__ void block_find_pivot_bucket_rep(uint32_t* __restrict__ s_hist,
  ...
  if (t < 256) {
#pragma unroll
    for (int r = 0; r < HIST_REP; r++) x += s_hist[t * HIST_REP + r];
    if constexpr (CLEAR) {
#pragma unroll
      for (int r = 0; r < HIST_REP; r++) s_hist[t * HIST_REP + r] = 0u;
    }
#pragma unroll
    for (int off = 1; off < WAVE_SIZE; off <<= 1) {
