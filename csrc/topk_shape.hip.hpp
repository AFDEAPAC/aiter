...
static inline ShapeParams derive_shape_params(int M, int N, int K, float margin_override,
  ...
  int S = sample_s_override > 0 ? align_sample_s(sample_s_override) : derive_sample_s_for_n(M, N, K, margin);
  if (!sample_stride_exact(N, S)) {
    ...
    if (g_s_repair_search != 0 && best > 0) S = best;
    else if (sample_stride_exact(N, repaired) || !sampling_geometry_ok(N, S)) S = repaired;
  }
