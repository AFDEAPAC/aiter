...
printf "%6s %9s %24s %24s %8s\n" M N "PA_FOLD=0" "PA_FOLD=1" ratio
for MN in "2048 131072" "1024 131072" "512 131072" "256 262144" "128 524288" "64 1048576" "4096 131072" "16 524288"; do
  set -- $MN; A=(); B=()
  for r in 1 2 3; do
    hipcc -O3 -std=c++17 --offload-arch=gfx950 -Icsrc -DPA_FOLD=0 -o benchmark_topk benchmark_topk.hip.cpp 2>&1|grep -i error|head -2
    A+=("$(ks $1 $2)")
    hipcc -O3 -std=c++17 --offload-arch=gfx950 -Icsrc -DPA_FOLD=1 -o benchmark_topk benchmark_topk.hip.cpp 2>&1|grep -i error|head -2
    B+=("$(ks $1 $2)")
  done
