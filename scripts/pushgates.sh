#!/bin/bash
cd /topk
echo "### state under test ###"
git -C /topk log --oneline -1 | cat
git -C /aiter log --oneline -1 | cat
echo
echo "=== 0. export contract ==="
...
cd /topk
echo
echo "=== 1. fullverify: 350 cases, 8 stages ==="
STAGE=all python3 -u scripts/fullverify.py 2>&1 | grep -E "FAILURES|m=" | sed "s/^/  /"
echo
echo "=== 2. mod3: N%4==3 focused, 191 cases ==="
python3 -u scripts/mod3.py 2>&1 | grep -E "FAILURES|m=" | sed "s/^/  /"
echo
echo "=== 3. benchmark verify, 245 cases ==="
...
echo
echo "=== 4. aiter op_tests ==="
pip -q install tabulate 2>&1|tail -0
cd /aiter && timeout 2000 python3 -u op_tests/test_topk_select.py > /topk/log/op6.log 2>&1
echo "  exit=$?"; grep -iE "all hold" /topk/log/op6.log | sed "s/^/  /"
echo
echo "=== 5. style, exactly what CI runs ==="
pip -q install black ruff==0.16.0 2>&1|tail -0
echo "  aiter: $(black --version|head -1)  /  $(ruff --version)"
black --check . 2>&1|tail -2|sed "s/^/  /"
ruff check . 2>&1|tail -1|sed "s/^/  /"
echo
echo "=== 6. fallback rate ==="
